...
class Exp_BSI_PRETRAIN(Exp_Basic):
    def __init__(self, args):
        super(Exp_BSI_PRETRAIN, self).__init__(args)
    ...
    def train_epoch_batched(self, model, optimizer, train_loader, epoch):
        ...
        for batch_X, x_labels in tqdm(train_loader, total=len(train_loader)):
            ...
            if self.args.pretrain_method == "cluster":

                _,embedding = self.model(train_X,train=True)

                cluster_loss, diverge_loss_dist, diverge_loss_angle = self.cluster_loss(embedding, train_labels, update_average=True)
                loss_batch =    cluster_loss*self.args.cluster_attract_weight + \
                        diverge_loss_dist*self.args.cluster_repel_weight_dist + \
                        diverge_loss_angle*self.args.cluster_repel_weight_angle
                ...
                try:
                    loss_diverge_dist.append(diverge_loss_dist.item())
                    
                except:
                    loss_diverge_dist.append(diverge_loss_dist)
                                    
                try:
                    loss_diverge_angle.append(diverge_loss_angle.item())
                except:
                    loss_diverge_angle.append(diverge_loss_angle)
                loss_cluster_ = np.array(loss_cluster).mean(axis=0)
                loss_diverge_dist_ = np.array(loss_diverge_dist).mean(axis=0)
                loss_diverge_angle_ = np.array(loss_diverge_angle).mean(axis=0)


                self.cluster_loss_val = loss_cluster_
                self.diverge_loss_dist_val = loss_diverge_dist_
                self.diverge_loss_angle_val = loss_diverge_angle_

            else:

                loss_batch = self.pretrain(train_X,train_labels)
    ...
    def valid_batch(self, model, valid_loader):
        ...
        for batch_X, x_labels in tqdm(valid_loader, total=len(valid_loader)):
            ...
            y_true_embedding.append(batch_X[:-1].cpu().detach().numpy())
            ...
            if self.args.pretrain_method == "cluster":
                ...
                cluster_loss, diverge_loss_dist, diverge_loss_angle = self.cluster_loss(embedding, train_labels, update_average=False)
                loss =    cluster_loss*self.args.cluster_attract_weight + \
                        diverge_loss_dist*self.args.cluster_repel_weight_dist + \
                        diverge_loss_angle*self.args.cluster_repel_weight_angle
                ...
                loss = self.pretrain(train_X,train_labels)
